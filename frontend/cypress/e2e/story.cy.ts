...
describe('story page', () => {
  ...
  it('should create new node in the story', () => {
    getComponent('node-input-1', 'textarea').type(
      'first node'
    )
    getComponent('node-input-1', 'textarea').should(
      'have.value',
      'first node'
    )
    ...
    getComponent('node-input-2', 'textarea').type(
      'second node'
    )
    getComponent('node-input-2', 'textarea').should(
      'have.value',
      'second node'
    )
  })
  ...
  it('should add prop of type text in the story node', () => {
    ...
    getComponent('node-prop-1-testProp', 'input').type(
      'prop of type text',
      {
        force: true
      }
  ...
  it('should remove prop from the story node', () => {
    ...
    getComponent('node-prop-remove-1-testProp').click({
      force: true
    })
    getComponent('node-prop-remove-1-testProp').click({
      force: true
  ...
  it('should unlink a child node, when clicked on the unlink btn', () => {
    ...
    cy.get('.svelvet-anchor.input.connected').should(
      'not.exist'
    )
  })
})
